...
import {Button, Card, Table, TableBody, TableColumn, TableHeader, TableRow} from "@nextui-org/react";
import MessageTableCell from './MessageTableCell';
import {useMessages} from '@/hooks/useMessages';

type Props = {
    initialMessages: MessageDto[]; nextCursor?: string
}

const MessageTable = ({initialMessages, nextCursor}: Props) => {

    const {
        columns, isOutbox, isDeleting, deleteMessage, selectRow, messages, loadMore, loadingMore, hasMore
            ...
            <Card>
                ...
                >
                    ...
                    <TableBody items={messages} emptyContent='No messages for this container'>
                        {(item) => (<TableRow key={item.id} className='cursor-pointer'>
                                {(columnKey) => (<MessageTableCell
                                        item={item}
